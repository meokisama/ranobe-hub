...
const renderButton = (href, label, { color = "#111111", textColor = "#ffffff" } = {}) => `
...
export const buildAdminNotifyEmail = (subscriberEmail) => {
...
export const buildConfirmationEmail = (email, isReactivation = false) => {
  ...
  const bodyHtml = `
    ...
    ${renderButton(process.env.FRONTEND_URL, "Khám phá thư viện")}
...
export const buildNewBookEmail = (email, bookTitle) => {
  ...
  const bodyHtml = `
    ...
    ${renderButton(process.env.FRONTEND_URL, "Đọc ngay")}
  `;
  const text = [
