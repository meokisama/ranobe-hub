...
interface BakaScreenProps {
  /** Alt text for the mascot — the only thing distinguishing 404 from maintenance. */
  imageAlt: string;
  /** Optional note rendered in a card below the title. */
  note?: string;
  /**
   * `fixed` for the 404 route, which overlays the site chrome; `relative` for the
   * maintenance page, which replaces it.
   */
  position?: "fixed" | "relative";
}

/** Shared full-screen "baa~ka~" mascot screen behind /404 and /maintenance. */
export function BakaScreen({ imageAlt, note, position = "relative" }: BakaScreenProps) {
  return (
    <main
      className={cn(
        "flex w-full items-center justify-center overflow-hidden bg-gradient-to-br from-[#fff5ec] via-[#ffe9e0] to-[#ffd9e4] px-6 py-16",
        position === "fixed" ? "fixed inset-0 z-40" : "relative min-h-screen",
      )}
    >
      ...
      >
        ...
        {note && (
          ...
          >
            <p className="text-center text-sm leading-relaxed text-rose-900/80 sm:text-[15px]">{note}</p>
          </motion.div>
        )}
      </motion.div>
    </main>
